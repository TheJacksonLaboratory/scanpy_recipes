def gen_qc(raw_adata):
    if raw_adata.uns['genome'].lower().startswith('m'):
        mt_query = sc.queries.mitochondrial_genes('useast.ensembl.org', 'mmusculus')
    else:
        mt_query = sc.queries.mitochondrial_genes('useast.ensembl.org', 'hsapiens')
...
def run_qc(adata, min_cells=3, min_genes=200, min_counts_per_cell=500, min_counts_per_gene=3,
    ...
    redux = adata.copy()
    redux = redux[redux.obs['sequencing_saturation'] > sequencing_saturation]
